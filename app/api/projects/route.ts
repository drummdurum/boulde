...
export async function POST(request: Request) {
  ...
  return NextResponse.json({ project: await createUserProject(user.id, { name: body.name, location: body.location, grade: body.grade as ClimbingGrade, note: typeof body.note === "string" ? body.note : "" }) }, { status: 201 });
}
