...
export async function POST(request: Request) {
  ...
  const name = body.get("name"); const placeId = body.get("placeId"); const grade = body.get("grade"); const note = body.get("note"); const imageFile = body.get("image");
  const place = typeof placeId === "string" ? placeById(placeId) : undefined;
  if (typeof name !== "string" || !name.trim() || !place) return NextResponse.json({ error: "Udfyld projektnavn og vælg et gyldigt sted." }, { status: 400 });
  if (typeof grade !== "string" || !grades.has(grade)) return NextResponse.json({ error: "Vælg en gyldig grade." }, { status: 400 });
  let image: string | undefined;
  if (imageFile instanceof File && imageFile.size > 0) {
    if (!imageFile.type.startsWith("image/")) return NextResponse.json({ error: "Vælg en gyldig billedfil." }, { status: 400 });
    if (imageFile.size > 8 * 1024 * 1024) return NextResponse.json({ error: "Billedet må højst fylde 8 MB." }, { status: 400 });
    const extension = imageFile.type.split("/")[1]?.replace("jpeg", "jpg").replace(/[^a-z0-9]/gi, "") || "jpg";
