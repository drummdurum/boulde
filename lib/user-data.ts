...
import type { ClimbingGrade, ClimbingProject, ClimbingSession, ClimbingType, Post, ProjectMedia, User } from "@/types";
...
type ProjectNode = { id: string; name: string; location: string; grade: ClimbingGrade; attempts: number; lastAttempt: string; note: string; status: ClimbingProject["status"]; progress: number; visible?: boolean };
...
function publicProject(row: ProjectNode): ClimbingProject { return { id: row.id, name: row.name, location: row.location, grade: row.grade, attempts: neo4jNumber(row.attempts), lastAttempt: row.lastAttempt, note: row.note, status: row.status, progress: neo4jNumber(row.progress), visible: row.visible === true }; }
...
export async function createUserProject(userId: string, input: { name: string; location: string; grade: ClimbingGrade; note: string; visible?: boolean }) {
  const result = await db.executeQuery(`MATCH (u:User {id: $userId}) CREATE (u)-[:WORKS_ON]->(p:Project { id: $id, name: $name, location: $location, grade: $grade, note: $note, visible: $visible, attempts: 0, lastAttempt: 'Ikke forsøgt endnu', status: 'Ny', progress: 0, createdAt: datetime() }) RETURN p`, { userId, id: randomBytes(12).toString("hex"), name: input.name.trim(), location: input.location.trim(), grade: input.grade, note: input.note.trim(), visible: input.visible === true }, { database, routing: "WRITE" });
...
export async function canViewProject(userId: string, projectId: string) { const result = await db.executeQuery(`MATCH (owner:User)-[:WORKS_ON]->(p:Project {id: $projectId}) OPTIONAL MATCH (viewer:User {id: $userId})-[:FOLLOWS]->(owner) RETURN owner.id = $userId OR (p.visible = true AND viewer IS NOT NULL) AS allowed`, { userId, projectId }, { database }); return result.records[0]?.get("allowed") === true; }
export async function getProjectForViewer(userId: string, projectId: string) { const result = await db.executeQuery(`MATCH (owner:User)-[:WORKS_ON]->(p:Project {id: $projectId}) OPTIONAL MATCH (viewer:User {id: $userId})-[:FOLLOWS]->(owner) WITH owner, p, owner.id = $userId AS owns, viewer WHERE owns OR (p.visible = true AND viewer IS NOT NULL) RETURN p, owner, owns`, { userId, projectId }, { database }); const record = result.records[0]; if (!record) return null; const project = publicProject(record.get("p").properties as ProjectNode); const owner = record.get("owner").properties as { id: string; name: string; username: string }; return { ...project, note: record.get("owns") ? project.note : "", owner: { ...owner, initials: initials(owner.name) } }; }
...
export async function getProjectMedia(projectId: string) { const result = await db.executeQuery("MATCH (:Project {id: $projectId})-[:HAS_MEDIA]->(m:ProjectMedia) RETURN m ORDER BY m.createdAt DESC", { projectId }, { database }); return result.records.map(record => { const item = record.get("m").properties as Omit<ProjectMedia, "projectId" | "url"> & { storageKey: string; createdAt: { toString(): string } | string }; return { ...item, size: neo4jNumber(item.size) }; }); }
