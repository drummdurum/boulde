...
export async function getUserPosts(user: User) { const result = await db.executeQuery("MATCH (:User {id: $userId})-[:CREATED]->(p:Post) RETURN p ORDER BY p.createdAt DESC LIMIT 100", { userId: user.id }, { database }); return result.records.map(record => publicPost(record.get("p").properties as PostNode, user)); }
...
export async function getUserProjects(userId: string) { const result = await db.executeQuery("MATCH (:User {id: $userId})-[:WORKS_ON]->(p:Project) RETURN p ORDER BY p.createdAt DESC LIMIT 100", { userId }, { database }); return result.records.map(record => publicProject(record.get("p").properties as ProjectNode)); }
...
export async function getProjectMedia(projectId: string) { const result = await db.executeQuery("MATCH (:Project {id: $projectId})-[:HAS_MEDIA]->(m:ProjectMedia) RETURN m ORDER BY m.createdAt DESC LIMIT 100", { projectId }, { database }); return result.records.map(record => { const item = record.get("m").properties as Omit<ProjectMedia, "projectId" | "url"> & { storageKey?: string; publicUrl?: string; createdAt: { toString(): string } | string }; return { ...item, size: neo4jNumber(item.size) }; }); }

function initials(name: string) { return name.split(/\s+/).map(part => part[0]).join("").slice(0, 2).toUpperCase(); }
function publicSession(record: SessionRecord, viewerId?: string): ClimbingSession {
  ...
  const host = (record.get("host") as { properties: { id: string; name: string } }).properties;
  ...
  const accepted = (record.get("accepted") as Array<{ id?: string; name?: string }>).filter(user => user.id && user.name).map(user => ({ id: user.id!, name: user.name!, initials: initials(user.name!) }));
  const project = projectNode?.properties;
  const invitationStatus = record.get("invitationStatus") as ClimbingSession["invitationStatus"] | null;
  return { id: session.id, shareId: session.shareId, title: session.title, date: session.date, time: session.time, location: session.location, createdAt: session.createdAt.toString(), ...(project ? { project: { id: project.id, name: project.name, grade: project.grade } } : {}), host: { id: host.id, name: host.name, initials: initials(host.name) }, participants: [{ id: host.id, name: host.name, initials: initials(host.name) }, ...accepted, ...guests], ...(viewerId === host.id ? { viewerRole: "host" as const } : invitationStatus ? { viewerRole: "invitee" as const, invitationStatus } : {}) };
}
export async function createClimbingSession(userId: string, input: { title: string; date: string; time: string; location: string; projectId?: string; inviteeIds?: string[] }) {
  ...
  if (inviteeIds.length) await db.executeQuery(`MATCH (host:User {id: $userId})-[:HOSTS]->(s:ClimbingSession {shareId: $shareId})
    UNWIND $inviteeIds AS inviteeId
    MATCH (host)-[:CONNECTED_WITH]-(invitee:User {id: inviteeId})
    MERGE (invitee)-[invitation:INVITED_TO]->(s)
    ON CREATE SET invitation.status = 'pending', invitation.invitedAt = datetime()`, { userId, shareId, inviteeIds }, { database, routing: "WRITE" });
  return getSharedSession(shareId, userId);
}
export async function getSharedSession(shareId: string, viewerId?: string): Promise<ClimbingSession | null> {
  const result = await db.executeQuery(`MATCH (host:User)-[:HOSTS]->(s:ClimbingSession {shareId: $shareId})
    ...
    RETURN s, p, host, guests, accepted, invitation.status AS invitationStatus`, { shareId, viewerId: viewerId || null }, { database });
  const record = result.records[0]; if (!record) return null;
  return publicSession(record, viewerId);
}
export async function getUserSessions(userId: string): Promise<ClimbingSession[]> {
  const result = await db.executeQuery(`MATCH (u:User {id: $userId})
    ...
    ORDER BY s.date, s.time`, { userId }, { database });
  return result.records.map(record => publicSession(record, userId));
}
export async function respondToSessionInvitation(userId: string, shareId: string, status: "accepted" | "declined") {
  const result = await db.executeQuery(`MATCH (:User {id: $userId})-[invitation:INVITED_TO]->(s:ClimbingSession {shareId: $shareId})
    SET invitation.status = $status, invitation.respondedAt = datetime()
    RETURN s.shareId AS shareId`, { userId, shareId, status }, { database, routing: "WRITE" });
