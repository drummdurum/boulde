...
export function FeedPost({ post }: { post: Post }) {
  ...
  return <article className="overflow-hidden rounded-[26px] border border-line bg-limestone shadow-soft">
    ...
    {post.image && <div className="relative aspect-[4/3] overflow-hidden bg-pine">
      {post.isVideo ? <video src={post.image} controls playsInline preload="metadata" className="h-full w-full object-cover" aria-label={post.imageAlt || "Video vedhæftet opslag"} /> : <Image src={post.image} alt={post.imageAlt || "Billede vedhæftet opslag"} fill unoptimized={post.image.startsWith("/api/posts/")} sizes="(max-width: 1024px) 100vw, 600px" className={`object-cover ${post.id === "p3" ? "scale-110 object-left" : ""}`} priority={post.id === "p1"} />}
    ...
    <div className="p-4 sm:p-5">
      ...
      {commentsOpen && <div className="mt-3 border-t border-line pt-4">
        {post.comments.length > 0 ? post.comments.map(c => <div key={c.id} className="mb-3 flex gap-2"><Avatar user={c.author} size="sm" /><p data-no-translate className="rounded-2xl bg-sand px-3 py-2 text-xs leading-5 text-ink"><strong className="mr-1">{c.author.name}</strong>{c.body}</p></div>) : <p className="mb-3 text-xs text-muted">Vær den første til at skrive en kommentar.</p>}
