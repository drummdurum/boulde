...
export function Dashboard({ user, initialPosts, initialProjects }: { user: User; initialPosts: Post[]; initialProjects: ClimbingProject[] }) {
  ...
  return <main className="min-h-screen px-4 pb-28 pt-6 sm:px-6 lg:ml-[238px] lg:px-8 lg:pb-10 xl:px-10">
    <div className="mx-auto max-w-[1460px]">
      ...
      <div className="grid items-start gap-8 xl:grid-cols-[minmax(0,1.55fr)_minmax(330px,.85fr)]">
        <section aria-labelledby="feed-title"><div className="mb-4"><p className="text-xs font-extrabold uppercase tracking-[.16em] text-clay">Dit fællesskab</p><h2 id="feed-title" className="mt-1 text-2xl font-extrabold text-ink">Seneste på væggen</h2></div>{posts.length ? <div className="space-y-6">{posts.map(post => <FeedPost key={post.id} post={post} />)}</div> : <div className="grid min-h-80 place-items-center rounded-[26px] border border-dashed border-line bg-limestone p-8 text-center"><div><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-sand text-clay"><ImagePlus size={25} /></span><h3 className="mt-5 text-lg font-extrabold text-ink">Dit feed er klar</h3><p className="mx-auto mt-2 max-w-sm text-sm font-semibold leading-6 text-muted">Du har ingen opslag endnu. Del din første klatring, når du er klar.</p><button onClick={() => setCreateOpen(true)} className="mt-5 rounded-2xl bg-pine px-5 py-3 text-sm font-extrabold text-limestone">Opret dit første opslag</button></div></div>}</section>
