...
export function UserProfilePage({ user: initialUser, createdAt, posts, projects, followCounts }: { user: User; createdAt: string; posts: Post[]; projects: ClimbingProject[]; followCounts: { followers: number; following: number } }) {
  ...
  return <main className="min-h-screen px-4 pb-28 pt-5 sm:px-6 lg:ml-[238px] lg:px-8 lg:pb-10 xl:px-10">
    ...
    {editing && <EditProfileModal user={user} onClose={() => setEditing(false)} onSaved={updated => { setUser(updated); setEditing(false); router.refresh(); }} />}
  </main>;
}
